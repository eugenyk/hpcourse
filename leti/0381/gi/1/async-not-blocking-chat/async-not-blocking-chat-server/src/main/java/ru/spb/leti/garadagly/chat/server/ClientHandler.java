...
import static ru.spb.leti.garadagly.chat.common.Message.Msg;
...
public class ClientHandler {
    ...
    public ClientHandler(final AsynchronousSocketChannel socketChannel, final ChatServer server) {
    ...
    private class ReadHandler implements CompletionHandler<Integer, ByteArrayOutputStream> {
        ...
        public void completed(Integer result, ByteArrayOutputStream msg) {
            try {
                if (result == -1) {
                    socketChannel.close();
                    server.unregisterClient(ClientHandler.this);
                    logger.info("Прекратил слушать этого клиента:  {}", ClientHandler.this);
                    return;
                }
                ...
                while (true) {
                    ...
                    if (msg.size() >= msgLength && msgLength > 0) {
                        ...
                        if (!checkAndRun(receivedMsg)) {
                            server.broadcast(ClientHandler.this, receivedMsg);
                        }
        ...
        private boolean checkAndRun(byte[] receivedMsg) {
            try {
                ...
                if (isCommand(from.getText())) {
                    CommandExecutor.INSTANCE.addTask(from.getText(), ClientHandler.this);
                    return true;
                }
