...
int VersionState::makeRaw(int version, VersionState::State state) {
	version = version << NUM_STATE_BITS;
        ...
        //Сделайте наименьшие биты версии, отражающие состояние
	for(int i = NUM_STATE_BITS-1; i >= 0; i--) 
...
// Применение:
// * Вы должны использовать ProbeBound-> load для получения необработанного значения.
// Когда у вас есть исходное значение, вы можете получить фактические значения, используя эти статические функции.
// Ex:
// ProbeBound p (10);
// int rawB = p-> load ();
// printf («Оценка зонда равна% d \ n», ProbeBound :: getBound (raw));
...
bool NBHashTable::insert(NBType k) {
	...
	while (true) {
        ...
        //Получает текущее состояние ковша версия и разбивает его на две части
        ...
        //Выполняет сравнение и свопа для изменения состояния (пуст -> занят)
    ...
    // Обновление значения ключа ведра
	getBucketValue(hashIndex, i)->key = k;
	
    while (true) {
	
                // установка состояния ведра  как visible
        getBucketValue(hashIndex, i)->vs->set(version, VersionState::State::VISIBLE);
		

        //обновление probe bound ведра
 
        conditionallyRaiseBound(hashIndex, i);
		
    
         // установка состояния ведра  как INSERTING
        ...
        //Находится ли текучее ведро не находится в состоянии коллизии
		if (getBucketValue(hashIndex, i)->vs->load() != collidedVs.load()) {
			return true;
		}
        
        //Если вспомогательная операция вернулась с ошибкой, обновите bound, увеличьте версию и опустошите ведро
		if (!r) {
...
//i - число отскоков от h, так что i = 0 при первом запуске
//ver_i - это версия в ковше [h, i]
bool NBHashTable::assist(NBType key, int h, int i, int ver_i) {
	int max = getProbeBound(h);
	
	
        //Просмотр всех возможных количеств прыжков
	...
	for(int j = 0; j < max; j++) {
		...
		VSTATE state_j = VersionState::getState(rawVS);
                ...
                // Если это состояние вставляется, и онj имеет тот же ключ, который мы пытаемся вставить
		...
		state_j = VersionState::getState(rawVS);
                ...
                //Если мы обнаружим, что наше значение уже является членом(записано), тогда выйд немедленно
