    ...
    class SkipListMapHdrTest: public HashMapHdrTest
    {
        ...
        void test()
        {
...

            {
                ...
                for ( int i = 0; i < nLimit; ++i ) {
                    int nKey = arrItem[i];
                    CPPUNIT_ASSERT( m.get_with( gp, nKey, less()));
                    CPPUNIT_ASSERT( !gp.empty());
                    CPPUNIT_CHECK( gp->first == nKey );
                    CPPUNIT_CHECK( gp->second.m_val == nKey * 2 );
                    gp.release();
                    CPPUNIT_ASSERT( m.extract_with(gp, nKey, less()));
                    CPPUNIT_ASSERT( !gp.empty());
                    CPPUNIT_CHECK( gp->first == nKey );
                    CPPUNIT_CHECK( gp->second.m_val == nKey * 2 );
                    gp.release();
                    CPPUNIT_CHECK( !m.get_with( gp, nKey, less()));
                    CPPUNIT_CHECK( !m.extract_with( gp, nKey, less()));
                    CPPUNIT_CHECK( gp.empty());
                }
                CPPUNIT_ASSERT( m.empty());
