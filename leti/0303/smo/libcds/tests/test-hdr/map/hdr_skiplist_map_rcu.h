...
namespace map {
    ...
    class SkipListMapRCUHdrTest: public HashMapHdrTest
    {
        ...
        struct extract_assign_functor {
            void operator()( extract_item& dest, typename Map::value_type& val ) const
            {
                dest.nKey = val.first;
                dest.nVal = val.second.m_val;
            }
        };
        ...
        void test()
        {
...

            {
                ...
                for ( int i = 0; i < nLimit; ++i ) {
                    ...
                    value_type * pExt = m.extract( nKey );
                    CPPUNIT_ASSERT( pExt != NULL );
                    CPPUNIT_CHECK( pVal == pExt );
                    CPPUNIT_CHECK( pExt->first == nKey );
                    CPPUNIT_CHECK( pExt->second.m_val == nKey * 2 );

                    CPPUNIT_CHECK( m.get( nKey ) ==  NULL );
                    CPPUNIT_CHECK( m.extract(nKey) ==  NULL );
                }
                CPPUNIT_ASSERT( m.empty() );
                ...
                for ( int i = 0; i < nLimit; ++i ) {
                    int nKey = arrItem[i]   ;
                    rcu_lock l;
                    value_type * pVal = m.get_with( nKey, less() );
                    CPPUNIT_ASSERT( pVal != NULL );
                    CPPUNIT_CHECK( pVal->first == nKey );
                    CPPUNIT_CHECK( pVal->second.m_val == nKey * 2 );

                    value_type * pExt = m.extract_with( nKey, less() );
                    CPPUNIT_ASSERT( pExt != NULL );
                    CPPUNIT_CHECK( pVal == pExt );
                    CPPUNIT_CHECK( pExt->first == nKey );
                    CPPUNIT_CHECK( pExt->second.m_val == nKey * 2 );

                    CPPUNIT_CHECK( m.get_with( nKey, less() ) ==  NULL );
                    CPPUNIT_CHECK( m.extract_with(nKey, less()) ==  NULL );
                }
                CPPUNIT_ASSERT( m.empty() );
                ...
                for ( int i = 0; i < nLimit; ++i ) {
                    rcu_lock l;
                    value_type * pGet = m.get_min();
                    CPPUNIT_ASSERT( pGet != NULL );
                    CPPUNIT_CHECK( pGet->first == i );
                    CPPUNIT_CHECK( pGet->second.m_val == i * 2 );

                    value_type * pVal = m.extract_min();
                    CPPUNIT_ASSERT( pVal != NULL );
                    CPPUNIT_CHECK( pVal == pGet );
                ...
                {
                    rcu_lock l;
                    CPPUNIT_CHECK( m.extract_min() ==  NULL );
                ...
                for ( int i = 0; i < nLimit; ++i )
                    CPPUNIT_ASSERT( m.insert(arrItem[i], arrItem[i]*2) ) ;
                for ( int i = 0; i < nLimit; ++i ) {
                    CPPUNIT_ASSERT( m.extract_min( exi, extract_assign_functor<Map>()));
                    CPPUNIT_CHECK( exi.nKey == i );
                    CPPUNIT_CHECK( exi.nVal == i * 2 );
                }
                CPPUNIT_ASSERT( m.empty() );
                CPPUNIT_CHECK( !m.extract_min(exi, extract_assign_functor<Map>()));
                ...
                for ( int i = nLimit-1; i >= 0; --i ) {
                    rcu_lock l;
                    value_type * pGet = m.get_max();
                    CPPUNIT_ASSERT( pGet != NULL );
                    CPPUNIT_CHECK( pGet->first == i );
                    CPPUNIT_CHECK( pGet->second.m_val == i * 2 );

                    value_type * pVal = m.extract_max();
                    CPPUNIT_ASSERT( pVal != NULL );
                    CPPUNIT_CHECK( pVal == pGet );
                    CPPUNIT_CHECK( pVal->first == i );
                    CPPUNIT_CHECK( pVal->second.m_val == i * 2 );
                ...
                {
                    rcu_lock l;
                    CPPUNIT_CHECK( m.extract_max() ==  NULL );
                ...
                for ( int i = 0; i < nLimit; ++i )
                    CPPUNIT_ASSERT( m.insert(arrItem[i], arrItem[i]*2) ) ;
                for ( int i = nLimit-1; i >= 0; --i ) {
                    CPPUNIT_ASSERT( m.extract_max( exi, extract_assign_functor<Map>()));
                    CPPUNIT_CHECK( exi.nKey == i );
                    CPPUNIT_CHECK( exi.nVal == i * 2 );
                }
                CPPUNIT_ASSERT( m.empty() );
                CPPUNIT_CHECK( !m.extract_max(exi, extract_assign_functor<Map>()));
            }

            CPPUNIT_MSG( PrintStat()(m, "End of test") ) ;
        }
