...
namespace set {
    ...
    protected:
        ...
        void test_skiplist_()
        {
            ...
            // extract/get test
            {
                ...
                // extract_min
                {
                    ...
                    while ( !s.empty() ) {
                        ...
                        nMin = gp->nKey;
                        CPPUNIT_CHECK( nMin == nPrevKey + 1 );
                        gp.release();

                        CPPUNIT_CHECK( s.extract_min( gp )) ;
                        CPPUNIT_ASSERT( !gp.empty());
                        CPPUNIT_CHECK( gp->nKey == nMin );
                ...
                // extract_max
                {
                    ...
                    while ( !s.empty() ) {
                        ...
                        CPPUNIT_ASSERT( !gp.empty() );
                        nMax = gp->nKey;
                        CPPUNIT_CHECK( nMax == nPrevKey - 1 );
                        gp.release();

                        CPPUNIT_CHECK( s.extract_max( gp )) ;
                        CPPUNIT_ASSERT( !gp.empty() );
                        CPPUNIT_CHECK( nMax == gp->nKey );
                        CPPUNIT_CHECK( gp->nKey == nPrevKey - 1 );
                        CPPUNIT_CHECK( (*gp).nVal == (nPrevKey - 1) * 2 );
                        nPrevKey = gp->nKey;
                    }
            ...
            CPPUNIT_MSG( PrintStat()( s, "After extract tests" ));
        }
