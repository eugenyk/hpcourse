    ...
    class SkipListSetHdrTest: public HashSetHdrTest
    {
        ...
        void test()
        {
            ...
            // extract test
            {
                ...
                for ( int i = 0; i < nLimit; ++i ) {
                    int nKey = arrRandom[i] ;
                    CPPUNIT_ASSERT( s.extract_with(gp, nKey, less_predicate() )) ;
                    CPPUNIT_ASSERT( !gp.empty()) ;
                    CPPUNIT_CHECK( gp->nKey == nKey );
                    CPPUNIT_CHECK( gp->nVal == nKey );
                    CPPUNIT_CHECK( !s.get_with(gp, nKey, less_predicate() ));
                    gp.release();
                }
                CPPUNIT_ASSERT( s.empty() );
                ...
                CPPUNIT_ASSERT( s.empty() );
                CPPUNIT_CHECK( !s.get_min(gp));
                CPPUNIT_ASSERT( gp.empty() );
                CPPUNIT_CHECK( !s.get_max(gp));
                CPPUNIT_ASSERT( gp.empty() );
            ...
            CPPUNIT_MSG( PrintStat()(s, "After extract") ) ;
        }
